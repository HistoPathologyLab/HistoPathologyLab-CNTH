...
app.post('/api/saveDoctor', async (req, res) => {
  try {
    ...
    console.log('Access Token Used:', accessToken); // Log the token being used
    await saveDoctor(req, res, accessToken);
  } catch (error) {
    console.error('Error in /api/saveDoctor:', error.message);
...
app.post('/api/removeDoctor', async (req, res) => {
  try {
    ...
    console.error('Error in /api/removeDoctor:', error.message);
